...
(function($) {
    ...
    $.jqplot.use_excanvas = (($.browser.msie && $.browser.version < 9.0) && !$.jqplot.support_canvas()) ? true : false;
    ...
    function jqPlot() {
        ...
        this.onMouseMove = function(ev) {
		 ...
		 // 如果是IE，小于50毫秒不触发事件
            if ($.browser.msie) {
                if (typeof ($.jqplot._lastMoved) == 'undefined') {
                    $.jqplot._lastMoved = new Date();
                    $.jqplot._lastMoved.add(-1, "minute");
                }
                var i = new Date() - $.jqplot._lastMoved;
                if (i < 50) return;
                $.jqplot._lastMoved = new Date();
            }
