...
(function (window) {
    ...
    var EventDegree = function () { }


    function setEventDegree(func) {
        EventDegree = func;
    }
    //var EventDegree = function (input) { }
    ...
    function powerConfig(args) {
        ...
        for (var q in defaults) {
            if (defaults.hasOwnProperty(q)) {
                tmpConfig[q] = defaults[q];
            }
        }
        for (var p in args)
            if (args[p] != null) tmpConfig[p] = args[p];
        return tmpConfig;
    }
    ...
    function writeMultiPassObjectOrg(oid, cfg) {
        ...
        if (IsNewest) {
            if (IsIE) {
                ...
                if (navigator.userAgent.indexOf("MSIE") > -1) {
                    document.getElementById(objId + oid).attachEvent("EventTab", EventTab, true);
                    document.getElementById(objId + oid).attachEvent("EventReturn", EventReturn, true);
                    document.getElementById(objId + oid).attachEvent("EventDegree", EventDegree, false);

                } else {
                    document.getElementById(objId + oid).addEventListener("EventTab", EventTab, true);
                    document.getElementById(objId + oid).addEventListener("EventReturn", EventReturn, true);
                    document.getElementById(objId + oid).addEventListener("EventDegree", EventDegree, false);
                }
            } else {
    ...
    function DealWithPwds(formName, ts, pwdKeys, idList) {
        var password;
        var pwdValues = "";
        ...
        if (typeof idList != "undefined") {
            var idListName = idList.split("@");
            for (i = 0; i < count; i++) {
                var powerpassName = idListName[i];
